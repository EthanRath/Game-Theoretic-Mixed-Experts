...
class BasicBlock(nn.Module):
    ...
    def forward(self, dic):
        ...
        mem[pos] 		= getattr(leak, 'l'+str(pos)) *mem[pos].cuda() + delta_mem
        mem_thr 		= (mem[pos].cuda()/getattr(threshold, 't'+str(pos))) - 1.0
        ...
        spike[pos] 		= spike[pos].cuda().masked_fill(out.bool(),t-1)
        out_prev  		= out.clone()
	
		#dropout1
        out_prev 		= out_prev * mask[pos].cuda()
		
		#conv2+identity
        delta_mem 		= self.residual[3](out_prev) + self.identity(inp)
        mem[pos+1] 		= getattr(leak, 'l'+str(pos+1))*mem[pos+1].cuda() + delta_mem
        ...
        spike[pos+1]	= spike[pos+1].cuda().masked_fill(out.bool(),t-1)
        out_prev  		= out.clone()
		...
		for t in range(self.timesteps):
			...
			for l in range(len(self.pre_process)):
				...
				if isinstance(self.pre_process[l], nn.Conv2d):
					...
					self.mem[l] 	= getattr(self.leak, 'l'+str(l))*self.mem[l].cuda() + delta_mem
					mem_thr 		= (self.mem[l].cuda()/getattr(self.threshold, 't'+str(l))) - 1.0
					...
					self.spike[l-1] 	= (self.spike[l-1].cuda()).masked_fill(out.bool(),t-1)
					out_prev  		= out.clone()
			...
			if not find_max_mem:
				if len(self.classifier)>1:
					self.mem[pos+l+1] 		= self.mem[pos+l+1].cuda()+ self.classifier[l+1](out_prev)
				else:
					self.mem[pos] 			= self.mem[pos].cuda() + self.classifier[0](out_prev)
